...
class WithEquityBcolzDailyBarReader(WithEquityDailyBarData, WithTmpDir):
    """
    ...
    # allows WithEquityBcolzDailyBarReaderFromCSVs to call the `write_csvs`
    # method without needing to reimplement `init_class_fixtures`
    ...
    @classmethod
    def init_class_fixtures(cls):
        super(WithEquityBcolzDailyBarReader, cls).init_class_fixtures()
...
class WithEquityBcolzDailyBarReaderFromCSVs(WithEquityBcolzDailyBarReader):
    """
...
class WithBcolzEquityMinuteBarReader(WithEquityMinuteBarData, WithTmpDir):
    ...
    cls.bcolz_minute_bar_ctable, and cls.bcolz_equity_equity_minute_bar_reader
    class level fixtures.
    ...
    - `cls.bcolz_equity_equity_minute_bar_reader` is a minute bar reader
       pointing to the directory that was just written to.
    ...
    def init_class_fixtures(cls):
        ...
        cls.bcolz_equity_equity_minute_bar_reader = \
            BcolzEquityMinuteBarReader(p)


class WithAdjustmentReader(WithEquityBcolzDailyBarReader):
    """
    ...
    make_adjustment_writer_equity_daily_bar_reader() -> pd.DataFrame
        ...
        ``WithEquityBcolzDailyBarReader``. This should probably not be
...
class WithDataPortal(WithAdjustmentReader,
    ...
    trading env, `cls.bcolz_equity_equity_minute_bar_reader`,
        ...
        if self.DATA_PORTAL_FIRST_TRADING_DAY is None:
            if self.DATA_PORTAL_USE_MINUTE_DATA:
                self.DATA_PORTAL_FIRST_TRADING_DAY = (
                    self.bcolz_equity_equity_minute_bar_reader.
                    first_trading_day)
